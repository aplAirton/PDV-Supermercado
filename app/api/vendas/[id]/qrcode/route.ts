...
export async function GET(
  ...
  try {
    ...
    console.log(`[qrcode] Buscando QR code para venda ID: ${vendaId}`)
    ...
    if (!cupomLink || cupomLink.length === 0) {
      console.log(`[qrcode] QR Code não encontrado para venda ${vendaId}`)
      return NextResponse.json({ error: 'QR Code não encontrado para esta venda' }, { status: 404 })
    }

    const dados = cupomLink[0]
    console.log(`[qrcode] QR code encontrado para venda ${vendaId}:`, dados.public_token.substring(0, 8) + '...')
