...
import { Plus, Edit, Trash2, Search, Loader2, Package, Scale, Hash, DollarSign, AlertTriangle, CheckCircle, X, Save, Camera, Barcode, Calculator, Settings, Info, FileText, MapPin, Calendar, Tag } from "lucide-react"
...
export default function ProdutosPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    preco: "",
    preco_kilo: "",
    unidade_medida: "unidade" as 'unidade' | 'kilo' | 'grama' | 'litro' | 'mililitro',
    peso_liquido: "",
    estoque: "",
    estoque_minimo: "",
    custo_compra: "",
  ...
  const [activeTab, setActiveTab] = useState<'basico' | 'estoque' | 'financeiro' | 'outros'>('basico')
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          preco_kilo: formData.preco_kilo ? Number.parseFloat(formData.preco_kilo) : null,
          peso_liquido: formData.peso_liquido ? Number.parseFloat(formData.peso_liquido) : null,
          estoque: Number.parseInt(formData.estoque),
          estoque_minimo: Number.parseInt(formData.estoque_minimo),
          custo_compra: formData.custo_compra ? Number.parseFloat(formData.custo_compra) : null,
          margem_lucro: formData.margem_lucro ? Number.parseFloat(formData.margem_lucro) : null,
        }),
      })
                  ...
                  <tr key={produto.id} className="product-row">
                      ...
                      <div className="name-info">
                        ...
                        {produto.peso_liquido && (
                          <small className="weight-info">
                            {produto.peso_liquido} {produto.unidade_medida === 'kilo' ? 'kg' : produto.unidade_medida === 'grama' ? 'g' : produto.unidade_medida === 'litro' ? 'L' : produto.unidade_medida === 'mililitro' ? 'ml' : 'un'}
                          </small>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="product-unit">
                      <div className="unit-display">
                        {produto.unidade_medida === 'kilo' && <Scale size={14} />}
                        {produto.unidade_medida === 'grama' && <Scale size={14} />}
                        {produto.unidade_medida === 'litro' && <Package size={14} />}
                        {produto.unidade_medida === 'mililitro' && <Package size={14} />}
                        {produto.unidade_medida === 'unidade' && <Hash size={14} />}
                        <span>{produto.unidade_medida === 'kilo' ? 'Kg' : produto.unidade_medida === 'grama' ? 'g' : produto.unidade_medida === 'litro' ? 'L' : produto.unidade_medida === 'mililitro' ? 'ml' : 'Un'}</span>
                      </div>
                    </td>
                    <td className="product-price">
                      <div className="price-info">
                        <span className="main-price">R$ {Number(produto.preco).toFixed(2)}</span>
                        {produto.preco_kilo && (
                          <small className="kilo-price">R$ {Number(produto.preco_kilo).toFixed(2)}/kg</small>
                        )}
                      </div>
                    </td>
                    <td className="product-stock">
                      <div className="stock-info">
                        <span className={`stock-value ${produto.estoque <= produto.estoque_minimo ? 'low-stock' : 'good-stock'}`}>
                          {produto.estoque}
                        </span>
                      ...
                      <div className="form-group">
                        ...
                        <div className="input-with-icon">
                          ...
                          <input
                            ...
                            required
                          />
                          <button type="button" className="input-btn" title="Escanear código">
                            <Camera size={16} />
                          </button>
                        </div>
                      </div>
                      ...
                      {(formData.unidade_medida === 'kilo' || formData.unidade_medida === 'grama') && (
                        <div className="form-group">
                          ...
                          <input
                            ...
                            placeholder={`Peso em ${formData.unidade_medida === 'kilo' ? 'kg' : 'g'}`}
                          />
                        </div>
                      )}
                        ...
                        <div className="form-group">
                          ...
                          <div className="input-with-icon">
                            ...
                            <input
                              ...
                              onChange={(e) => setFormData({ ...formData, preco_kilo: e.target.value })}
                              placeholder="0,00"
                            />
                          </div>
                        </div>
                  ...
                  <div className="form-section">
                    ...
                    <div className="stock-summary">
                      <div className="summary-item">
                        ...
                        <span className={`summary-value ${(Number(formData.estoque) || 0) <= (Number(formData.estoque_minimo) || 0) ? 'warning' : 'success'}`}>
                          {(Number(formData.estoque) || 0) <= (Number(formData.estoque_minimo) || 0) ? 'Estoque Baixo' : 'Estoque Adequado'}
                        </span>
                      </div>
                      <div className="summary-item">
                        <span className="summary-label">Disponível:</span>
                        <span className="summary-value">{Number(formData.estoque) || 0} unidades</span>
                      </div>
                    </div>
                  </div>
                )}
                      ...
                      <div className="form-group">
                        ...
                        <div className="input-with-icon">
                          ...
                          <input
                            ...
                            onChange={(e) => setFormData({ ...formData, custo_compra: e.target.value })}
                            placeholder="0,00"
                          />
                        </div>
                      </div>
                        ...
                        <div className="summary-grid">
                          {formData.custo_compra && (
                            <div className="summary-item">
                              ...
                              <span className="summary-value">R$ {Number(formData.custo_compra).toFixed(2)}</span>
                            </div>
                          )}
                          {formData.margem_lucro && (
                            <div className="summary-item">
                              <span className="summary-label">Margem:</span>
                              <span className="summary-value">{Number(formData.margem_lucro).toFixed(2)}%</span>
                            </div>
                          )}
                          {formData.preco && formData.custo_compra && (
                            <div className="summary-item">
                              ...
                              <span className="summary-value success">
                                R$ {(Number(formData.preco) - Number(formData.custo_compra)).toFixed(2)}
                              </span>
                            </div>
                          )}
                        </div>
